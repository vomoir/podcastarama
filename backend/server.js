...
import { analyzeAudio, simpleTrimAudio, trimAudio, getAudioMetadata } from './audioAnalyzer.js';
...
app.post('/api/episodes/:id/trim', async (req, res) => {
  try {
    ...
    const { startTime, endTime, segments } = req.body;
    const requestedSegments = Array.isArray(segments)
      ? segments
      : (startTime !== undefined && endTime !== undefined ? [{ start: startTime, end: endTime }] : []);

    if (!requestedSegments.length) {
      return res.status(400).json({ error: 'A trim range or segments array is required' });
    }

    const validSegments = requestedSegments
    ...
    if (!validSegments.length) {
      return res.status(400).json({ error: 'No valid trim ranges were provided' });
    }
    ...
    const originalDuration = (await getAudioMetadata(episode.audioPath)).duration;
    const removedDuration = validSegments.reduce((total, seg) => total + (seg.end - seg.start), 0);

    if (validSegments.length === 1) {
      await simpleTrimAudio(episode.audioPath, validSegments[0].start, validSegments[0].end, trimmedPath);
    } else {
      await trimAudio(episode.audioPath, validSegments, trimmedPath, 'remove');
    }
    ...
    const newDuration = Math.max(0, Math.round((originalDuration || 0) - removedDuration));
